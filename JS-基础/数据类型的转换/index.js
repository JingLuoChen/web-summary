...
* 数据转换分为显示转换和隐式转换
*
* 显示转换：常见的显示转换方法有 Boolean、Number、String等等
*
...
*
* parseInt()函数可解析一个字符串，从0位置开始查看每个字符，直到找到第一个非有效的字符为止，最后并返回一个整数
* 即 会把小数转换成整数
*
...
* 3)、Number(string)
*
* Number()函数转换的是整个值，而不是部分值
*
* parseInt 和 parseFloat 方法只转换第一个无效字符之前的字符串，因此 "1.2.3" 会分别转换为 1 和 1.2
*
 ...
 * Object类型
 *
 * 是由key-value聚合的数据集合，即属性的集合
 *
 * JS对象主要可以分为两大类
 *
 * 内置对象(内部类): Array、Boolean、RegExp、Date、Math、Number、String等等
 *
 * 宿主对象: js运行环境所提供的对象，BOM中的window、DOM中的document
 *
...
* -----------------------------------------------------------------------
*
* 如果类数组对象或者可遍历的对象要转换，可以使用Array.from() -> 转为数组
*
...
* 四、数组Object转为String
*
* 通过使用join()或者toString()方法转换为String
*
...
* 即可以看下undefined的使用场景
*
* 变量被声明了，但没有赋值， 该变量 -> undefined
*
* 调用函数时，应该提供的参数没有提供，该参数 -> undefined
*
* 对象没有赋值属性，该属性的值 -> undefined
*
